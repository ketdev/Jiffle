...
INLINE int __write_w32(const char* buf, FILE* fp) {
    static WORD attr_olds[2] = {0xFFFF, 0xFFFF}, attr_old;
    static int first = 1;
    int type;
                            ...
                            switch (v[i]) {
                                case 3:
                                    ...
                                    FillConsoleOutputCharacter(
                                        handle, ' ', csize, coord, &written);
                                    FillConsoleOutputAttribute(
                                        handle, csbi.wAttributes, csize, coord,
                                        &written);
                                    SetConsoleCursorPosition(
                                        handle, csbi.dwCursorPosition);
                                    csbi.dwSize.X = 132;
                                    SetConsoleScreenBufferSize(handle,
                                                               csbi.dwSize);
                                    csbi.srWindow.Right =
                                        csbi.srWindow.Left + 131;
                                    SetConsoleWindowInfo(handle, TRUE,
                                                         &csbi.srWindow);
                                    break;
                                case 5:
                ...
                case 'm':
                    ...
                    for (i = 0; i <= n; i++) {
                        if (v[i] == -1 || v[i] == 0)
                            ...
                            if ((v[i] - 30) & 1) attr |= FOREGROUND_RED;
                            if ((v[i] - 30) & 2) attr |= FOREGROUND_GREEN;
                            if ((v[i] - 30) & 4) attr |= FOREGROUND_BLUE;
                        } else if (v[i] == 39)
                            attr = (~attr & BACKGROUND_MASK);
                        else if (v[i] >= 40 && v[i] <= 47) {
                            attr = (attr & FOREGROUND_MASK);
                            if ((v[i] - 40) & 1) attr |= BACKGROUND_RED;
                            if ((v[i] - 40) & 2) attr |= BACKGROUND_GREEN;
                            if ((v[i] - 40) & 4) attr |= BACKGROUND_BLUE;
                        } else if (v[i] == 49)
                            attr = (~attr & FOREGROUND_MASK);
                        else if (v[i] == 100)
                            attr = attr_old;
...
#else
#define __write_w32 fputs

#endif

INLINE int ansi_fputs_out(const char* string) {
    return __write_w32(string, stdout);
}

INLINE int ansi_fputs_err(const char* string) {
    return __write_w32(string, stderr);
}
